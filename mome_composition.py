...
#             draw.rectangle([0, y0, text_x, y1], fill=color)
#             draw.rectangle([text_x + text_width, y0, width, y1], fill=color)
#     else:
#         bar_width = width // n
#         for i, color in enumerate(colors):
#             x0 = i * bar_width
#             x1 = (i + 1) * bar_width if i < n - 1 else width
#             draw.rectangle([x0, 0, x1, text_x], fill=color)
#             draw.rectangle([x0, text_x + text_width, x1, bars_height], fill=color)
...
# def add_text(stage, current_image):
#     """
...
#         - horizontal (bool)
#         - x, y (optional): explicit position
...
#     text = stage['text']
#     font_size = stage.get('font_size', 100)
#     gradient = stage.get("gradient", False)
#     if gradient: 
#         color1 = stage.get('color1', (255, 0, 0, 255))
#         color2 = stage.get('color2', (0, 0, 255, 255))
#     else: 
#         color1 = color2 = stage.get("color", (0,0,0,255))
#     horizontal = stage.get('horizontal', True)
#     font_path = stage.get('font_path', get_random_font())
...
#     x = stage.get('x')
#     y = stage.get('y')
#     centered = stage.get('centered', True)  # Default is centered
#     center_x = stage.get('center_x', False)
#     center_y = stage.get('center_y', False)
...
#     if stage.get('bars', False):
#         bars_height_ratio = stage.get('bars_height_ratio', 0.4)
#         bars_height = int(text_height * bars_height_ratio)
#         bars_top = y + int(text_height * 0.5)
...
#     return current_image, {'x': x, 'y': y, 'text_width': text_width, 'text_height': text_height}

def add_text(stage, current_image):
    """
    Adds text to the image at a specified location or centered by default.
    If x or y is a float between 0 and 1, it is interpreted as a fraction of the image width/height.
    Accepts:
        - text (str)
        - font_size (int)
        - color1, color2 (for gradient)
...
def vhs_scanline(params, current_image):
    """
...
def crop_stage(stage, current_image):
    """
    ...
    Args:
        ...
        current_image (PIL.Image.Image): The image to crop.
